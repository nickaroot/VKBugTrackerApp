...
//        reportInfo = [ReportInfo](),
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return reportComments.count + staticRowsOffset
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (error) {
            ...
            if (indexPath.item == 0) {
                ...
                cell.contentView.frame.size.height += 11
...
//                cell.contentView.frame.size.height += 11
            ...
            } else if (indexPath.item == staticRowsOffset - 1) {
                
                let cell = Bundle.main.loadNibNamed("ReportActionsTableViewCell", owner: self, options: nil)?.first as! ReportActionsTableViewCell
                cell.selectionStyle = .none
                
                let productIndex = products.index(where: { $0.id == productId })
                
                if products[productIndex!].coverImage != nil {
                    cell.productCover.image = products[productIndex!].coverImage!
                } else {
                    DispatchQueue.main.async {
                        let imageData = try? Data(contentsOf: products[productIndex!].coverUrl!)
                        
                        products[productIndex!].coverImage = UIImage(data: imageData!)
...
//            } else if (indexPath.item >= staticRowsOffset + reportInfo.count) {
                ...
                if reportComment.avatarImage == nil {
                    DispatchQueue.main.async {
                        ...
                        self.reportComments[indexPath.item - self.staticRowsOffset].avatarImage = UIImage(data: imageData)!
                        cell.authorAvatar.image = self.reportComments[indexPath.item - self.staticRowsOffset].avatarImage
                    }
                } else {
                ...
                cell.contentView.frame.size.height += 7
                ...
                if (indexPath.item == staticRowsOffset + reportComments.count - 1) {
                    cell.separatorInset = UIEdgeInsetsMake(0, 1000, 0, 0)
                }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if (indexPath.item >= self.staticRowsOffset) {
            let alertController = UIAlertController(title: nil, message: nil, preferredStyle: .actionSheet)
            let item = indexPath.item - self.staticRowsOffset
...
//                        self.title = crumb.text!
                        let productId = Int((crumb["href"]?.matchingStrings(regex: "product=([0-9]+)"))![0][1])!
                        self.productId = productId
                        
                        let productTitle = crumb.text!
                        let product = Product(id: productId, title: productTitle, coverUrl: nil, coverImage: nil)
                        let newProduct = products.first { $0.id == productId } == nil
                        ...
                        if newProduct {
                            ...
                            do {
                                ...
                                let productContents = try String(contentsOf: URL(string: "https://vk.com/bugtracker?act=product&al=0&al_id=\(userId!)&id=\(productId)")!, encoding: .windowsCP1251)
                                
                                let productDoc = try HTML(html: String(describing: productContents), encoding: .windowsCP1251)
                                
                                let productCoverUrl = URL(string: productDoc.at_css(".bt_prod_one_photo__img")!["src"]!)
                                
//                                guard let imageData = try? Data(contentsOf: productCoverUrl!) else {
//                                    print("Cover Loading Error...")
//                                    return
//                                }
//
                                let productIndex = products.index(where: { $0.id == productId })
//
//                                products[productIndex!].coverImage = UIImage(data: imageData)
                                products[productIndex!].coverUrl = productCoverUrl
...
//            self.reportComments.append(reportComment)
    ...
    func removeComment(id: String, hash: String, item: Int) {
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            self.reportComments.remove(at: item - self.staticRowsOffset)
